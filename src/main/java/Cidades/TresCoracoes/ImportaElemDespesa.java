...
public class ImportaElemDespesa extends Util {
    ...
    public static void init(int anoSonner) {
        ...
        Integer empresa, ficha, fonteRecurso, caFixo, caVariavel, ctUnidadeOrcamentaria, ctFuncao;
        ...
        try {
            ...
            stmt = con.prepareStatement(
                    "Select SEQ_CT_UNIDADE_ORCAMENTARIA, SEQ_CT_FUNCAO  " +
                            " From dbo.CT_ORCAMENTO_DESPESA " +
                            " where ANO_ORCAMENTO_DESPESA = ? " +
                            " Order by 1 ");
            stmt.setInt(1, anoSonner);
            rs = stmt.executeQuery();
            while (rs.next()) {
                ctUnidadeOrcamentaria = rs.getInt(1);
                ctFuncao = rs.getInt(2);
